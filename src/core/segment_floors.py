...
def segment_floors(pcd):
    ...
    hist, bin_edges = np.histogram(z_values, bins=int((max(z_values) - min(z_values)) / 0.05))
    ...
    peaks = find_peaks(hist, height=0.01*len(pcd.points), distance=10)
    print(peaks)
...
def identify_floor_ceiling_pairs(peaks, bin_edges):
    ...
    peak_z_values.sort()
    print(peak_z_values,"peak_z_values")

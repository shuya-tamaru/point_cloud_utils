...
def extract_directional_planes(pcd, normals):
    ...
    x_positive = (abs_normals[:, 0] > dominant_threshold) & (normals_array[:, 0] > 0)
    x_negative = (abs_normals[:, 0] > dominant_threshold) & (normals_array[:, 0] < 0)
    y_positive = (abs_normals[:, 1] > dominant_threshold) & (normals_array[:, 1] > 0)
    y_negative = (abs_normals[:, 1] > dominant_threshold) & (normals_array[:, 1] < 0)
    z_positive = (abs_normals[:, 2] > dominant_threshold) & (normals_array[:, 2] > 0)
    z_negative = (abs_normals[:, 2] > dominant_threshold) & (normals_array[:, 2] < 0)
